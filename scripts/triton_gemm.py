...
# if os.getenv("BES_GROUP_SIZE", "0") == "0":
#     group_size = [1, 2]
# elif os.getenv("BES_GROUP_SIZE", "0") == "1":
#     group_size = [4, 8]
# else:
#     group_size = [16]
...
def make_matmul_kernel(configurations):
    ...
    custom_data_storage=os.path.abspath(
        os.path.join(os.path.dirname(__file__), "gemm_data_bao_lhs_10_stop_power_of_two_3")
    ),
    )
    @triton.jit
    def matmul_kernel(
...
def matmul(a, b, configurations, activation=""):
    # Check constraints.
    ...
    matmul_kernel = make_matmul_kernel(configurations)
    try:
        matmul_kernel[grid](
            a, b, c,  #
            M, N, K,  #
...
# Matrix Multiplication Kernel Definiton
@triton.jit
